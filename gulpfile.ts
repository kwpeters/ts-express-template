...
import { ActionPriority, diffDirectories } from "./dev/depot/diffDirectories";
...
export function copyAssets(): Promise<void> {
    return promiseResult.toPromise(runCopyAssets());
}


async function runCopyAssets(): Promise<Result<undefined, string>> {
    const srcDir = new Directory("src", "assets");
    const dstDir = new Directory(distDir, "assets");
    await dstDir.ensureExists();

    const diffItems = await diffDirectories(srcDir, dstDir, false);

    const actionPromises =
        diffItems.map((curDiffItem) => {
            return curDiffItem.actions(ActionPriority.SyncLeftToRight)
            .then((actions) => {
